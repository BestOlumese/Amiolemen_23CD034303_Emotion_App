# Emotion Detection Web App (StudentName_MATNO)
...
4. Prepare training data (if training):
   - Create `data/train/<label>/*.jpg` and `data/val/<label>/*.jpg`
   - Run `python model.py` to train and save model as models/emotion_model.h5
...
## Notes
- Ensure `models/emotion_model.h5` exists before starting app.
